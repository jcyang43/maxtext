...
Create 2 nodepools with the same configurations for multi-slice training on the GKE cluster. If you want to run it with single slice, change the `replicas` on this [line](./tpu_v6e_with_gcp_monitoring.yaml#L13) to `1`.
```
...
On this [line](./tpu_v6e_with_gcp_monitoring.yaml#L38) of the config file, replace the placeholder image with the image you just built (`<your-docker-registry-path>/maxtext_base_tpu:latest`).

### 6. Set the training dataset
...
If you don't have a training dataset or want to try with synthetic data, replace `dataset_path=${DATASET_PATH}` with `dataset_type=synthetic` on this [line](./tpu_v6e_with_gcp_monitoring.yaml#L51).
