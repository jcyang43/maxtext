...
This guide provides an overview on how to integrate Google's [workload observability agent](us-west2-docker.pkg.dev/gce-ai-infra/workload-observability/model-workload-observability) container into your MaxText training workload. 
...
2. A GKE cluster ready in your project. For our example, we use a v6e TPU cluster with a 4x4 topology. If you choose a different cluster, please remember to modify your configurations accordingly. 
...
Create 2 nodepools with the same configurations for multi-slice training on the GKE cluster. If you want to run it with single slice, change the `replicas` on this [line](./tpu_v6e_with_gcp_monitoring.yaml#13) to 1.
```
...
### 3. Install the jobset plugin on your cluster
Install the [jobset](https://github.com/kubernetes-sigs/jobset) plugin for running multi-slice.
...
### 5. Build your MaxText Docker image 
In the project root directory, run 
```
bash docker_build_dependency_image.sh DEVICE=tpu

# tag and push your image 
docker image tag maxtext_base_image:latest <your-docker-registry-path>/maxtext_base_tpu:latest
docker push <your-docker-registry-path>/maxtext_base_tpu:latest
```

In the [config file](./tpu_v6e_with_gcp_monitoring.yaml#38), replace the placeholder image with the image you just built (`<your-docker-registry-path>/maxtext_base_tpu:latest`).
